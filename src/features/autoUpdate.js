...
async function autoUpdate(contents) {
    return new Promise((resolve, reject) => {
        contents.on('dom-ready', async () => {
            ...
            let res = await fetch('https://github.com/TomatoTTV/KCV3Test/releases/latest/download/latest.yml')
            ...
            if (text != version) {
                ...
                contents.send('message', 'No update. Starting KClient...');
                resolve(false)
            }
        })
    })
...
async function downloadUpdate(contents) {
    ...
    const pUrl = "https://api.github.com/repos/TomatoTTV/KCV3Test/releases";
